...
const RootComponent = (props) => {
  ...
  const addProductToCart = (product) => {
    const updatedProducts = cart.products.map((p) => {
      if (p.id === product.id) {
        return { ...p, qty: p.qty + 1, price: product.price * (p.qty + 1) }
      } else {
        return p
      }
    })
    const totalPrice = updatedProducts.reduce((acc, p) => acc + p.price, 0)
    setCart({ products: updatedProducts, totalPrice })
  }
    ...
    const updatedProducts = cart.products.map((p) => {
      if (p.id === product.id) {
        return {
          ...
          price: product.price * Math.max(0, p.qty - 1)
        }
      } else {
        return p
      }
